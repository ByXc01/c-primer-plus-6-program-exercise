/* File: convert_vla.c
 * Author: ByXc
 * About: Digital convert picture, variable-length array, eliminate distortion
 * Version: 3.0
...
void initialization(int, int, FILE *, int (*)[*]);   // 给整数数组初始化
void eliminate_distortion(int, int, int (*)[*]);     // 消除失真
...
int main(void)
{
    ...
    eliminate_distortion(i_lines, i_cols, ia_number);                              // 消除失真
...
void eliminate_distortion(int i_lines, int i_cols, int (* iap_digital)[i_cols])
{
    int i_r, i_c;
    double db_sum;

    for (i_r = 1; i_r < i_lines - 1; i_r++)
    {
        for (i_c = 1; i_c < i_cols + 1; i_c++)
        {   // 判断元素与上下左右元素的差
            if (iap_digital[i_r][i_c] - iap_digital[i_r-1][i_c] >= 1 && iap_digital[i_r][i_c] - iap_digital[i_r+1][i_c] >= 1 && iap_digital[i_r][i_c] - iap_digital[i_r][i_c-1] >= 1 && iap_digital[i_r][i_c] - iap_digital[i_r][i_c+1])
            {
                // 上下左右的元素和
                db_sum = iap_digital[i_r-1][i_c] + iap_digital[i_r+1][i_c] + iap_digital[i_r][i_c-1] + iap_digital[i_r][i_c+1];
                db_sum /= 4.0;                                      // 平均值
                db_sum += ((int)(db_sum * 10) % 10 >= 5) ? 1 : 0;   // 四舍五入
                iap_digital[i_r][i_c] = db_sum;                     // db_sum 小数部分会被截断（如果有的话）
            }
        }
